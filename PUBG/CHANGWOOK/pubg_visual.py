...
def get_matchID(api_key, tournamentID):
    pubg = PUBG(api_key)
...
def track(api_key, matchId, teamName,tournmentID = "as-pgs1gf",):
    api_key = api_key
    pubg = PUBG(api_key)
    ...
    # track data 그리기
    current_match = pubg.match('8c4c4c09-ba79-410e-bd75-b60bb83629c4', shard="tournament")
...
def landing(api_key, matchId, tournmentID = "as-pgs1gf"):
    api_key = api_key
    pubg = PUBG(api_key)
    ...
    current_match = pubg.match(matchId, shard="tournament")
    telemetry = current_match.get_telemetry()
    positions = telemetry.player_positions()
